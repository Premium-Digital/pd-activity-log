...
use PdActivityLog\Logger;

class ActivityLogsTable extends \WP_List_Table {
    private $logger;
    ...
    protected function column_default($item, $column_name) {
        if ($column_name === 'post') {
            ...
            if (in_array($objectType, ['post', 'page', 'product'])) {
                $post = get_post($objectId);
                if ($post) {
                    $title    = esc_html(get_the_title($post));
                    $editLink = esc_url(get_edit_post_link($objectId));
                    $label = ucfirst($objectType); // np. Post, Page, Product
                    return sprintf('<a href="%s" target="_blank">%s (%s)</a>', $editLink, $title, $label);
                } else {
                    return sprintf('<em>%s ID: %d (deleted)</em>', ucfirst($objectType), $objectId);
                }
            }
            ...
            if ($objectType === 'term') {
                ...
                if ($term && !is_wp_error($term)) {
                    ...
                    return sprintf('<em>Term ID: %d (deleted)</em>', $objectId);
                }
            }
